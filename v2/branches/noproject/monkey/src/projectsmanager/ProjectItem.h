...
virtual ~ProjectItem();
	...
	//will connect self to existing of build menu, set it's "enabled" property, add not standard actions
	virtual void addActionsToMenu ();
	//will disconnect self from actions, remove not standard, that was added
	virtual void removeActionsFromMenu ();
...
private:
	...
	/*
	Command, that will used for make a action.
	If there will be more that one command, it's must be separated by '\n'
	If string is empty - action is disabled
	*/

private slots:
	//all plugins, that have project settings, should have this member
