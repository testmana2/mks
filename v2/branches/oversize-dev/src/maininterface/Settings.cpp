...
{
	connect( this, SIGNAL( maxRecentFilesChanged( int ) ), MenuBar::self(), SLOT( updateRecentFiles() ) );
	connect( this, SIGNAL( maxRecentProjectsChanged( int ) ), MenuBar::self(), SLOT( updateRecentProjects() ) );
}
//
...
{
	if ( i != value( "Recents/MaxFiles" ).toInt() )
	{
		setValue( "Recents/MaxFiles", i );
		emit maxRecentFilesChanged( i );
	}
}
//
int Settings::maxRecentFiles() const
{
	return value( "Recents/MaxFiles", 15 ).toInt();
}
//
void Settings::setMaxRecentProjects( int i )
{
	if ( i != value( "Recents/MaxProjects" ).toInt() )
	{
		setValue( "Recents/MaxProjects", i );
		emit maxRecentProjectsChanged( i );
	}
}
//
int Settings::maxRecentProjects() const
{
	return value( "Recents/MaxProjects", 15 ).toInt();
}
