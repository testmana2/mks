...
void TextEditor::focusInEvent ( QFocusEvent* e )
{
	...
	if ( mReplaceDialog && mReplaceDialog->workingEdit() != this )
...
{
	emit pasteAvailable( !qApp->clipboard()->text().isEmpty() );
}
//
...
void TextEditor::setTabSize( int i )
{
	if ( i == mTabSize )
		return;
	mTabSize = i;
	setTabStopWidth( fontMetrics().width( " " ) * mTabSize );
...
//
bool TextEditor::open( const QString& fp, QTextCodec* c )
{
	if ( !fp.isNull() )
		setFilePath( fp );
	if ( mFilePath.isNull() )
...
//
bool TextEditor::save( bool a, QTextCodec* c )
{
...
//
bool TextEditor::pasteAvailable()
{
	return !qApp->clipboard()->text().isEmpty();
}
//
void TextEditor::openFile( const QString& s, QTextCodec* c )
{
	open( s, c );
}
//
void TextEditor::saveFile( bool b, QTextCodec* c )
{
	save( b, c );
}
//
void TextEditor::closeFile( bool b )
{
	save( true );
	clear();
	setFileOpen( false );
...
// SLOTS
void TextEditor::gotoLine( int i )
{
	setTextCursor( getLineCursor( i ) );
}
//
void TextEditor::highlightLine( int i )
{
	gotoLine( i );
	QTextCursor c = textCursor();
