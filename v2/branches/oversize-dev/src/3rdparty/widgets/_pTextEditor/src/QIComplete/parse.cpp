...
bool Parse::getTypeOfExpression(const QString &expr, Expression * exp, Scope * scope)
{
	...
	while (--start >= first)
	{
		...
		else
		{
			...
			case '>':		/* pointer access */
			case ':':		/* static access */
				if ((*start == '>' && (start - 1 >= first && (start - 1)[0] == '-'))
				        || (*start == ':'
				            && (start - 1 >= first && (start - 1)[0] == ':')))
				{
					if (in_ident && extract_ident)
					{
						const char *ident = start + 1;
...
extract:
	...
	if (exp->access != AccessStatic)
	{
		...
		type = getTypeOfToken(ident, NULL, scope, scanForFuncdef(start));
...
Expression Parse::parseExpression(QString expression, Scope * scope)
{
	Expression exp;
	exp.access = ParseError;

	int len = expression.length();	/* strlen() returns the size without the NULL-byte */
	if (len < 1)
		return exp;
...
void Parse::getScopeAndLocals(Scope * sc, const QString &expr, const QString &ident)
{
	...
	QString command =
	    ...
	    smallTagsFilePath + "\" \"" + sourceFilePath + '\"';
