...
public:
	...
	*/
	...
	static Tree *buildIncludeTree(Tree *start, const QString &filename, Tree* = NULL);

	/* free()'s all memory allocated of a Tree* */
	void freeTree();

	/* NULL pointer checks must be performed by the caller */
	void writeTreeToFile(QFile &file);
