	...
	: QFrame( parent )
{
	...
	// Completion
	comp = new InitCompletion( this );
	comp->setTempFilePath( qApp->applicationDirPath() );
	comp->setCtagsCmdPath( "ctags" );
	comp->addIncludes( QStringList() << "/home/pasnox/Desktop/qt4ds-src-2.0.0alone"
		<< "/home/shares/local/lib/qt4.1.2/include"
		<< "/usr/include" );
	comp->addMacros( QStringList() << "Q_OBJECT" << "slots" << "signal"
		<< "__attribute_pure__" << "__attribute_malloc__"
		<< "THROW" << "KDE_DEPRECATED" );
}
//
...
bool Completion::prepareCompletion()
{
	...
	*/
	...
	// go to first item
	lwItems->scrollToTop();
	QList<Tag> TagList = comp->startParse( mEditor->toPlainText().left( mEditor->textCursor().position() ) );
	foreach ( Tag tag, TagList )
	{
		if ( tag.isFunction )
			( void ) new QListWidgetItem( tag.name + tag.parameters + " -- " + tag.longName, lwItems, itOthers );
		else
			( void ) new QListWidgetItem( tag.name + tag.parameters + " -- " + tag.longName, lwItems, itVariables );
	}
