...
Tree* Tree::buildIncludeTree(Tree * start, const QString &filename, Tree * root)
{
	...
	QString fullname;
	QFile *fd = parent->getFiledescriptor(filename, fullname);
	if (!fd)
		return NULL;

	QString buffer = fd->readAll();
	fd->close();
	delete fd;
	...
	while ((pos = rx.indexIn(buffer, pos)) != -1)
	{
		...
		/* get/save full filename for this #include */
		QFile *temp_fd = parent->getFiledescriptor(include, fullname);
		...
		if (root->findInTree(fullname) == NOT_FOUND)
		{
			child = new Tree;
			if (child)
			{
				...
				Tree::buildIncludeTree(child, include, root);
			}
		}
	}
...
void Tree::writeTreeToFile(QFile &file)
{
	file.write( QString(name + '\n').toAscii());
	for (int i = 0; i < inherits.size(); i++)
		inherits[i]->writeTreeToFile(file);
}
