...
const QList<pTemplate> pTemplatesManager::defaultTemplates()
{
	...
	// C++ Projects
	<< pTemplate( "C++", pTemplate::ttProjects, "NoProject", "Special project type, alowing you to build projects, that have no own project engine (or it's not supported), by executing console commands.", ":/templates/icons/templates/empty.png", unTokenize( QStringList() << "$TEMPLATE_PATH$/C++/Projects/noproject.noproject"))
