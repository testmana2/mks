...
int DebuggerAt::pluginDataFromGdb(struct GNUDebuggerPluginStruct * debugger_struct)
{
	...
	switch(debugger_struct->iPluginCommand)
	{
		case DEBUGGER_AT:
		{
			if(debugger_struct->qsDataFromGdb.contains("at") && debugger_struct->qsDataFromGdb.contains("#"))
			{
				// decompilation
				QString nameFile;
				QString line;
