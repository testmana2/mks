...
void QsciAPIs::autoCompletionList(const QStringList &words, QStringList &wlist)
{
    ...
    {
        QString new_word = words[i];

        if (!lex->caseSensitive())
            new_word = new_word.lower();

        if (i < prev_words.count() && prev_words[i] != new_word)
            same_context = false;
    ...
    if (origin_len > 0)
    {
        ...
        while (start_new < end_new)
        {
            ...
            path.append(new_words[start_new]);
...
void QsciAPIs::lastPartialWord(const QString &word, QStringList &wlist) const
{
    ...
    while (it != prep->wdict.end())
    {
        ...
        addAPIEntries(it.data(), false, wlist);
...
void QsciAPIs::autoCompletionSelected(const QString &sel)
{
    ...
    QString owords = lst[1].mid(1, lst[1].length() - 2);
...
{
    const QString &api_word = api_words[idx];
    ...
    for (int i = 0; i < idx; ++i)
        org.append(api_words[i]);

    return QString("%1 (%2)").arg(api_word).arg(org.join(wsep));
}
