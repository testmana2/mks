...
{
    // Ignore if automatic auto-completion is disabled.
    if (acSource == AcsNone || wseps.isEmpty())
        return;
...
void QsciScintilla::callTip()
{
    ...
    while (loff > 0)
    {
        ...
        else if (ch == '(' && loff > 0)
        {
            ...
            {
                // The parenthesis is preceded by a word so find the start of
                // that word.
                lbuf[loff--] = '\0';

                while (loff >= 0)
                {
                    if (!isWordChar(lbuf[loff]) && isStartChar(lbuf[loff]))
                        break;
            ...
            // We are between parentheses that do not correspond to a call tip,
            // so reset the argument count.
            commas = 0;
        }
    }
    ...
    QString ct = apis->callTips(&lbuf[start], maxCallTips, commas);
    ...
    const char *cts = ct.toLatin1().data();

    ctpos = SendScintilla(SCI_POSITIONFROMLINE, linenr) + start;
    SendScintilla(SCI_CALLTIPSHOW, ctpos, cts);
...
{
    QString ct = lex->apis()->callTipsNextPrev(dir);

    if (ct.isNull())
        return;

    SendScintilla(SCI_CALLTIPSHOW, ctpos, ct.toLatin1().data());
}


// Return the list of words that should be considered when auto-completing.  If
// it is empty then auto-completion should be abandoned.  The list will only
// have more than one element if there is a lexer set and it defines start
// strings.  If so, then the last element might be empty if a start string has
// just been typed.
QStringList QsciScintilla::wordsForAutoCompletion()
{
    enum {
        Either,
        Separator,
    ...
    while (pos > 0)
    {
        ...
        // Strip any preceeding spaces (mainly around operators).
        char ch;
...
bool QsciScintilla::getSeparator(int &pos)
{
    int opos = pos;
...
QString QsciScintilla::getWord(int &pos)
{
    QString word;
    bool numeric = true;
    char ch;
...
char QsciScintilla::getCharacter(int &pos)
{
    if (pos <= 0)
        return '\0';
...
{
    QStringList words = wordsForAutoCompletion();

    if (words.isEmpty())
        return;

    // Get the last word's raw data and length.
    const QString &last = words.last();
    ...
    {
        const QsciAPIs *apis = lex->apis();

        if (apis)
            apis->autoCompletionList(words, cs, wlist);
    }

    if ((acs == AcsAll || acs == AcsDocument) && last_len > 0)
    {
        SendScintilla(SCI_SETSEARCHFLAGS,
                SCFIND_WORDSTART | (cs ? SCFIND_MATCHCASE : 0));
        ...
        for (;;)
        {
            ...
            if ((fstart = SendScintilla(SCI_SEARCHINTARGET, last_len, last_data)) < 0)
                break;
    ...
    SendScintilla(SCI_AUTOCSHOW, last_len, wlist.join(QChar(sep)).toLatin1().data());
}
...
int QsciScintilla::lineAt(const QPoint &pos)
{
...
int QsciScintilla::lineLength(int line)
{
...
QString QsciScintilla::selectedText()
{
    if (!selText)
        return QString();
...
QString QsciScintilla::text(int line)
{
...
void QsciScintilla::getCursorPosition(int *line, int *index)
{
    long pos = SendScintilla(SCI_GETCURRENTPOS);
...
int QsciScintilla::indentation(int line)
{
...
int QsciScintilla::currentIndent()
{
...
bool QsciScintilla::marginLineNumbers(int margin)
{
...
int QsciScintilla::marginMarkerMask(int margin)
{
...
bool QsciScintilla::marginSensitivity(int margin)
{
...
int QsciScintilla::marginWidth(int margin)
{
...
unsigned QsciScintilla::markersAtLine(int linenr)
{
...
int QsciScintilla::markerLine(int mhandle)
{
...
int QsciScintilla::markerFindNext(int linenr, unsigned mask)
{
...
int QsciScintilla::markerFindPrevious(int linenr, unsigned mask)
{
...
void QsciScintilla::setLexer(QsciLexer *lexer)
{
    ...
    if (lex)
    {
        ...
        // Force the preparation of any APIs which can take time.  We do it now
        // to (hopefully) avoid a delay when the next auto-completion list or
        // call tip is displayed.
        lex->apis();
    }
    else
    {
...
long QsciScintilla::posFromLineIndex(int line, int index)
{
...
void QsciScintilla::lineIndexFromPos(long pos, int *line, int *index)
{
    long lin = SendScintilla(SCI_LINEFROMPOSITION, pos);
    long linpos = SendScintilla(SCI_POSITIONFROMLINE, lin);
...
QString QsciScintilla::convertTextS2Q(const char *s)
{
    if (isUtf8())
...
bool QsciScintilla::isWordChar(char ch) const
{
...
int QsciScintilla::textHeight(int linenr)
{
