...
def usage(rcode = 2):
    ...
    """
    print "Usage:"
    print "    python configure.py [-h] [-c] [-d dir] [-j #] [-k] [-n dir] [-o dir] [-p 3|4] [-r] [-s] [-u] [-v dir]"
    print "where:"
    ...
    print "  -n dir  the directory containing the QScintilla header files [default %s]" % pyqt.qt_inc_dir
...
def check_qscintilla():
    ...
    sciglobal = os.path.join(opt_qsciincdir, "qsciglobal.h")
    ...
    if os.access(sciglobal, os.F_OK):
        ...
        if glob.glob(os.path.join(opt_qscilibdir, "*qscintilla2*")):
            ...
            sipconfig.inform("The QScintilla library could not be found in %s. If QScintilla is installed then use the -o argument to explicitly specify the correct directory." % opt_qscilibdir)
    else:
        sipconfig.inform("qsciglobal.h could not be found in %s. If QScintilla is installed then use the -n argument to explicitly specify the correct directory." % opt_qsciincdir)
...
def generate_code():
    ...
    if pyqt.pyqt_version >= 0x040000:
        ...
        mtype = pyqt4.QtGuiModuleMakefile
    else:
        argv.append("sip/qscimod3.sip")
        mtype = pyqt3.QtModuleMakefile
...
def main(argv):
    ...
    try:
        optlist, args = getopt.getopt(argv[1:], "hcd:j:kn:o:p:rsuv:")
