...
QsciScintilla::QsciScintilla(QWidget *parent)
    : QsciScintillaBase(parent),
      ...
      wchars(defaultWordChars), maxCallTips(-1), showSingle(false),
      modified(false), explicit_fillups(""), fillups_enabled(false)
{
...
void QsciScintilla::callTip()
{
    ...
    QString ct = apis->callTips(context, maxCallTips, commas);
...
// empty if a start string has just been typed.
...
{
    QStringList context = contextWords(SendScintilla(SCI_GETCURRENTPOS));
