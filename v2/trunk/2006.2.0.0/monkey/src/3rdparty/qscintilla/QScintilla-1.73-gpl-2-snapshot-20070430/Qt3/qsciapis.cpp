        ...
        QStringList &wlist)
{
    ...
    else
    {
        ...
        if (unambig)
            ...
            {
                // Remove the unambigious context.
                QString noc = with_context.at(i);
...
QString QsciAPIs::callTips(const QStringList &context, int maxnr, int commas)
{
    QString path;
    positionOrigin(context, path);

    // If we don't have a valid context then we could display call tips for
    // every function with that name - but we don't.
    if (path.isEmpty())
        return QString();
    ...
    // Work out the offset from the start of the API entry of the bit we want
    // to show in the call tip (ie. the function name and everything after).
    QString wsep = lex->autoCompletionWordSeparators().first();
